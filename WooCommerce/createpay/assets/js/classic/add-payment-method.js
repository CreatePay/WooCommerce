jQuery( document ).ready( function ( $ ) {
	...
	$( function () {
		...
		const hostedFields = new window.hostedFields.classes.Form(
			window.document.forms.add_payment_method,
			{
				autoSetup: true,
				autoSubmit: false,
				merchantID: localizeVars.merchantID,
				fields: {
					any: {
		...
		// Listen to place order button click on Add payment method form.
		$( '#place_order' ).on( 'click', async function ( event ) {
			...
			toggleCheckoutOverlay( window.document.forms.add_payment_method, true );

			// Tokenise the card details.
			await hostedFields.getPaymentDetails( {}, true ).then( async function ( result ) {
				...
				if ( result.success ) {
					...
					const deviceInformation = {
						...
						deviceScreenResolution: `${window.screen.width}x${window.screen.height}x${screen.colorDepth}`,
						deviceAcceptLanguage: navigator.languages ?? null,
					};

					const formData = new FormData();
					// Add the action and security code to the form data.
					formData.append( 'nonce', localizeVars.nonce );
					formData.append( 'action', 'add_new_saved_card' );

					// Add payment data.
					formData.append(
						'paymentData',
						JSON.stringify( {
							paymentType: 'add_new_saved_card',
					...
					fetch( localizeVars.ajaxurl, {
						...
						.then( async ( json ) => {
							if ( json.success ) {
								...
								if ( json.data.threeDSRequired ) {

									const threeDSData = JSON.parse( json.data.threeDSData );

									toggleCheckoutOverlay( window.document.forms.add_payment_method, false );

									await handle3DS( threeDSData ).then( function ( threeDSResult ) {
										if ( threeDSResult.paymentResult === 'verified' ) {
											// Continue place order button event.
											$( 'form#add_payment_method' ).append( '<input type="hidden" name="transactionUnique" value="' + threeDSResult.transactionUnique + '"/>' );
											$( 'form#add_payment_method' ).submit();
										} else {
											showCheckoutError( threeDSResult.message || '3D Secure authentication failed. Please try again.' );
											toggleCheckoutOverlay( window.document.forms.add_payment_method, false );
										}
									} ).catch( function () {
										showCheckoutError( 'An error occurred during 3D Secure authentication. Please try again.' );
										toggleCheckoutOverlay( window.document.forms.add_payment_method, false );
									} );
								}
							} else {
								showCheckoutError( json.data.message || 'There was a problem adding the card. Please try again.' );
								toggleCheckoutOverlay( window.document.forms.add_payment_method, false );
							}
						} )
						.catch( function () {
							showCheckoutError( 'An error occurred while processing the request. Please try again.' );
							toggleCheckoutOverlay( window.document.forms.add_payment_method, false );
						} );
				} else if ( result.invalid ) {
					showCheckoutError( 'Please check your card details and try again.' );
					toggleCheckoutOverlay( window.document.forms.add_payment_method, false );
				} else {
					showCheckoutError( 'There was a problem with the payment. Please contact support.' );
					toggleCheckoutOverlay( window.document.forms.add_payment_method, false );
				}
			} );
		} );
	 ...
	 *
	 * @param {*} threeDSData Three DS data.
	 * @return {Promise}
	 */
